...
export default function MobileTocNav({ items }: Props) {
  ...
  return (
    <div className="tablet:hidden">
      ...
      <button
        onClick={open}
        className={`fixed bottom-4 left-1/2 -translate-x-1/2 z-40 inline-flex items-center bg-cream border border-dark-green/10 rounded-full pl-5 pr-6 py-2 shadow-[0_-1px_4px_rgba(0,0,0,0.1)] w-[21.375rem] h-14 transition-opacity duration-300 ${showBar ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        aria-label="目次を開く"
      >
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute bottom-6 right-4 left-4 flex flex-col items-end gap-2">
            ...
            <div className="bg-[#f4faf0] border border-dark-green/10 rounded-3xl px-5 py-6 w-full shadow-[0_0_8px_rgba(0,0,0,0.16)]">
