...
プロジェクトの全体要件を定義したドキュメント
- プロジェクト概要
- サイト構造（全12ページ）
- データモデル設計
- 機能要件（フロントエンド・CMS管理機能）
- 非機能要件
- ビジネスロジック仕様

### 🏗️ [実装計画書](./implementation-plan.md)
技術スタックと実装手順を定義したドキュメント
- 技術スタック詳細
- 実装フェーズ（全12フェーズ）
- コード実装例
- 工数見積もり
...
### 🤖 [AI機能追加ロードマップ](./docs/ai-roadmap.md)
Webサイトローンチ後のAI機能追加計画
- AI対話コンシェルジュ（Plan A）
- ストーリー自動生成エンジン（Plan B）
- 画像アップロード簡易査定（Plan C）
- MCP対応による外部連携
- 実装スケジュール・コスト見積もり
- 現在の設計への影響と準備事項
...
**詳細**: [AI機能追加ロードマップ](./docs/ai-roadmap.md)参照
...
詳細は[実装計画書](./implementation-plan.md)の「4. 開発環境セットアップ手順」を参照してください。
...
詳細は[要件定義書](./requirements.md)の「3. データモデル設計」を参照してください。

## デプロイ

Vercelへのデプロイ手順は[実装計画書](./implementation-plan.md)の「5. デプロイ手順」を参照してください。
